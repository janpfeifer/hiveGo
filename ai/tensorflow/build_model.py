...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    input_names = [x.name for x in (
        is_training, learning_rate, unsupervised_loss_ratio, board_features, full_board, board_labels)]
    ...
    actions_piece = tf.placeholder(
        tf.float32, shape=[None, NUM_PIECE_TYPES], name='actions_piece')
    actions_labels = tf.placeholder(
        tf.float32, shape=[None], name='actions_labels')

    print('Action Inputs:')
    input_names = ["{}: {}".format(x.name, x.dtype)
                   for x in (actions_board_indices, actions_src_positions,
                             actions_tgt_positions, actions_is_move, actions_piece)]
    print('\t{}\n'.format('\n\t'.join(input_names)))
    ...
    #     tf.cast(tf.reshape(actions_predictions, [-1]), tf.float32), name='actions_pr   edictions')
