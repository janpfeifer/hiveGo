...
type Derived struct {
	...
	// It is cleared with Board.ClearNextBoardsCache.
	nextBoards []*Board
...
//
// For boards for which one no longer is examining the sub-boards, call ClearNextBoardsCache.
...
// ClearNextBoardsCache clear the cache of the boards created by taking each valid action.
// This is important to do for boards no longer being examined in the search (boards for
// older moves).
//
// See TakeAllActions.
func (b *Board) ClearNextBoardsCache() {
	b.Derived.nextBoards = nil
}
